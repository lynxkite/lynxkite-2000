...
def test_update_node_ids_uses_neighbors_when_params_are_empty():
    source = workspace.Workspace()
    source_load_connected = source.add_node(
        id="src-load-connected", title="Load", params={}
    )
    ...
    target_load_connected = target.add_node(
        id="tmp-load-connected", title="Load", params={}
    )
