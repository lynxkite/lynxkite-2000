...
from .opcontext import OpContext
...
def matplotlib_to_image(func):
    ...
    def wrapper(*args, **kwargs):
        func(*args, **kwargs)
        self, *rest = args
        if isinstance(self, OpContext):
            # If the first argument is an OpContext, we can use its node to determine the size of the figure.
            if self.node:
