...
function needsNodeInternalsUpdate(prevNode: any, nextNode: any) {
  ...
  if (endpointSignature(prevNode.data?.meta?.inputs) !== endpointSignature(nextNode.data?.meta?.inputs)) {
    return true;
  }
  if (
    endpointSignature(prevNode.data?.meta?.outputs) !== endpointSignature(nextNode.data?.meta?.outputs)
  ) {
    return true;
  }
  return false;
